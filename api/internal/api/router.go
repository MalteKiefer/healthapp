...
func NewServer(db *pgxpool.Pool, rdb *redis.Client, logger *zap.Logger, cfg *config.Config, ts *crypto.TokenService, version string) *Server {
	...
	userHandler := handlers.NewUserHandler(db, userRepo, logger, "/data/uploads", tokenService)
...
func (s *Server) setupRoutes() {
	...
	s.Router.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Legal consent (exempt from consent check via middleware)
			r.Post("/legal/accept", s.LegalHandler.HandleAcceptConsent)
			r.Get("/legal/pending", s.LegalHandler.HandlePendingConsent)
