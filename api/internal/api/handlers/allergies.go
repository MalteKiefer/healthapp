...
// HandleMigrateContent lazily backfills the content_enc column for an allergy
...
// PATCH /profiles/{profileID}/allergies/{allergyID}/migrate-content
func (h *AllergyHandler) HandleMigrateContent(w http.ResponseWriter, r *http.Request) {
	claims, ok := ClaimsFromContext(r.Context())
	if !ok {
		writeJSON(w, http.StatusUnauthorized, errorResponse("not_authenticated"))
		return
	...
	if err != nil {
		...
		h.logger.Error("get allergy for migrate", zap.Error(err))
		writeJSON(w, http.StatusInternalServerError, errorResponse("internal_error"))
		return
	}
	if existing.ProfileID != profileID {
	...
	if err := h.allergyRepo.SetContentEnc(r.Context(), allergyID, body.ContentEnc); err != nil {
